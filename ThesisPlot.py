...
Q2FullDense = """{0} 2 1 0.25
{1} 2 30 3.75
{2} 10 820 45.5556
{3} 101 25320 617.561""".format(40**3, 80**3, 160**3, 320**3)
...
def plot_regressions(num_list, times_list, deg, color):
    ...
    if deg == 1:
        ...
        plt.plot(x, c*x + d, color=color, label='Linear Fit (SSE = {0:.2e})'.format(float(res)), linestyle='solid')
    elif deg == 2:
        ...
        b, c, d = coefs
        x = np.linspace(min(num_list), max(num_list), 100)
        plt.plot(x, b*(x**2) + c*x + d, color=color, label='Quadratic Fit (SSE = {0:.2e})'.format(float(res), linestyle='dotted'))
    elif deg == 3:
        ...
        print(res)
        x = np.linspace(min(num_list), max(num_list), 100)
        plt.plot(x, a*(x**3) + b*(x**2) + c*x + d, color=color, label='Cubic Fit ($SSE = 0$)'.format(float(a)), linestyle='dashed')
...
def plot_basic_stuff():
    ...
    if matrix.shape[1] == 3:
        ...
        for deg in range(1, 4):
            plot_regressions(matrix[:,1], matrix[:,2], deg, color[deg-1])
        ...
        fig, ax = plt.subplots()
        ax.xaxis.set_major_formatter(FormatStrFormatter('%.0E'))
        ax.scatter(matrix[:,1], matrix[:,2], label='Data Points')
        plt.xlabel('Number of Objects')
